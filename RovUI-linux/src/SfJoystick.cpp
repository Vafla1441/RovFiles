...
void SfJoystick::timerEvent(QTimerEvent*)
{
    ...
    if (isConnected()) {
        ...
        m_servoZ = (int)sf::Joystick::getAxisPosition(0, m_joystickSettingsDialog.data()->servoZAxis());
        m_joystickSettingsDialog.data()->isServoZInvese() ? m_servoZ *= -1 : m_servoZ *= 1;
    ...
    if (isConnected()) {
        ...
        RovSingleton::instance()->control().cameraRotation[2] = static_cast<qint8>(m_servoZ);
        ...
        RovSingleton::instance()->control().cameraRotation[2] = static_cast<qint8>(0);
