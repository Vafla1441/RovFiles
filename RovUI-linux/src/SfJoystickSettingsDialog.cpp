...
SfJoystickSettingsDialog::SfJoystickSettingsDialog(QWidget* parent)
    ...
    , m_servoZList(new QComboBox(this))
    ...
    , m_servoZInv(new QCheckBox(this))
...
sf::Joystick::Axis SfJoystickSettingsDialog::servoZAxis()
{
    return m_servoZAxis;
}

bool SfJoystickSettingsDialog::isServoZInvese()
{
    return m_isServoZInv;
}

void SfJoystickSettingsDialog::setServoZAxis(sf::Joystick::Axis axis, bool inverse)
{
    m_settings->beginGroup("Joystick");
    m_settings->setValue("JoystickServoZAxis", static_cast<int>(axis));
    m_settings->setValue("JoystickServoZAxisInv", inverse);
    m_servoZAxis = axis;
    m_isServoZInv = inverse;
    m_settings->endGroup();
}
...
int SfJoystickSettingsDialog::pumpButton()
{
    return m_pumpButton;
}

void SfJoystickSettingsDialog::setPumpButton(int button)
{
    m_settings->beginGroup("Joystick");
    m_settings->setValue("JoystickPumpButton", button);
    m_pumpButton = button;
    m_settings->endGroup();
}

int SfJoystickSettingsDialog::lazerButton()
{
    return m_lazerButton;
}

void SfJoystickSettingsDialog::setLazerButton(int button)
{
    m_settings->beginGroup("Joystick");
    m_settings->setValue("JoystickLazerButton", button);
    m_lazerButton = button;
    m_settings->endGroup();
}
...
void SfJoystickSettingsDialog::setFiftyButton(int button)
{
    m_settings->beginGroup("Joystick");
    m_settings->setValue("fiftyButton", button);
    m_fiftyButton = button;
    m_settings->endGroup();
}

int SfJoystickSettingsDialog::twentyFiveButton()
{
    return m_twentyFiveButton;
}

void SfJoystickSettingsDialog::setTwentyFiveButton(int button)
{
    m_settings->beginGroup("Joystick");
    m_settings->setValue("twentyFiveButton", button);
    m_twentyFiveButton = button;
    m_settings->endGroup();
}
...
void SfJoystickSettingsDialog::readSettings()
{
    ...
    m_isServoZInv = m_settings->value("JoystickServoZAxisInv", false).toBool();
    m_servoZInv.data()->setChecked(m_isServoZInv);
...
void SfJoystickSettingsDialog::createLayout()
{
    ...
    addToLayout(m_servoZList.data(), QString(tr("Тяга на сервопривод 3:")), layout, m_servoZInv.data());
    ...
    m_servoZList.data()->setCurrentIndex(static_cast<int>(servoZAxis()));
...
{    
    ...
    QObject::connect(m_servoZList.data(), QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int index) {
        this->setServoZAxis(static_cast<sf::Joystick::Axis>(index), m_servoZInv.data()->isChecked());
    });
    ...
    QObject::connect(m_pumpButtonList.data(), QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int index) {
        this->setPumpButton(index);
    });
    ...
    QObject::connect(m_fiftyButtonList.data(), QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int index) {
        this->setFiftyButton(index);
    });

    QObject::connect(m_twentyFiveButtonList.data(), QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int index) {
        this->setTwentyFiveButton(index);
    });
    ...
    QObject::connect(m_servoZInv.data(), &QCheckBox::stateChanged, [this](int index) {
        Q_UNUSED(index)
        this->setZAxis(static_cast<sf::Joystick::Axis>(m_servoZList.data()->currentIndex()), m_servoZInv.data()->isChecked());
    });
